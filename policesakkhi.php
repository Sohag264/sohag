...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO policesakkhi (title, summary) VALUES (?, ?)");
...
// সামারি লোড করা
$query = "SELECT * FROM policesakkhi ORDER BY id DESC";
            ...
            <div id="titleList">
                <?php while ($row = $result->fetch_assoc()): ?>
                    <div class="list-item" onclick="showDetails('<?= addslashes($row['title']) ?>', '<?= htmlspecialchars(addslashes($row['summary'])) ?>')" data-title="<?= strtolower($row['title']) ?>">
                        ...
                        <a href="policesakkhiedit.php?id=<?= $row['id'] ?>" class="">Edit</a>
                    </div>
                <?php endwhile; ?>
            </div>
        </div>
