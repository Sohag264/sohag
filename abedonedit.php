...
if ($id > 0) {
    $stmt = $conn->prepare("SELECT * FROM abedon WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE abedon SET title = ?, summary = ? WHERE id = ?");
    $stmt->bind_param("ssi", $title, $summaryContent, $id);
    $stmt->execute();
    $stmt->close();

    header("Location: abedon.php"); // মূল পেজে ফিরে যান
    exit;
}
?>
...
<body>
    <div class="container">
        <h2>আবেদন/প্রতিবেদন এডিট করুন</h2>
        <form method="POST">
            ...
            <div class="form-group">
                <label for="summary">বিস্তারিত</label>
