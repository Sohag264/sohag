...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO abedon (title, summary) VALUES (?, ?)");
...
// সামারি লোড করা
$query = "SELECT * FROM abedon ORDER BY id DESC";
...
<body>
    ...
    <div class="container">
        ...
        <div class="title-section">
            <h3></h3>
            <button class="add-summary-btn" onclick="showPopup()">Create New</button>
            ...
            <div id="titleList">
                <?php while ($row = $result->fetch_assoc()): ?>
                    <div class="list-item" onclick="showDetails('<?= addslashes($row['title']) ?>', '<?= htmlspecialchars(addslashes($row['summary'])) ?>')" data-title="<?= strtolower($row['title']) ?>">
                        ...
                        <a href="abedonedit.php?id=<?= $row['id'] ?>" class="">Edit</a>
                    </div>
                <?php endwhile; ?>
            </div>
        </div>
