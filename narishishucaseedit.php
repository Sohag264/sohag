...
if ($id > 0) {
    $stmt = $conn->prepare("SELECT * FROM narishishucase WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE narishishucase SET title = ?, summary = ? WHERE id = ?");
    $stmt->bind_param("ssi", $title, $summaryContent, $id);
    $stmt->execute();
    $stmt->close();

    header("Location: narishishucase.php"); // মূল পেজে ফিরে যান
    exit;
}
?>
