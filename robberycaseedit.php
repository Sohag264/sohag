...
if ($id > 0) {
    $stmt = $conn->prepare("SELECT * FROM robberycase WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE robberycase SET title = ?, summary = ? WHERE id = ?");
    $stmt->bind_param("ssi", $title, $summaryContent, $id);
    $stmt->execute();
    $stmt->close();

    header("Location: robberycase.php"); // মূল পেজে ফিরে যান
    exit;
}
?>
